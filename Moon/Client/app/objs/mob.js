    ...
    MoonMob.prototype.NextInstruction = function () {
        ...
        if (this.PInstruction == "m") {
            ...
            if (prevInstruction != this.PInstruction || prevSpeed != this.Speed) {
                var anispeed = this.Speed;
                //anispeed = App.MoonGame.value_speed; //TEST
                this.CounterView = new mcounter_1.MCounter(action[this.DirectionView].length, anispeed, true);
            }
        }
    };
    MoonMob.prototype.CalcDirection = function () {
        var px = Math.abs(this.PParam2), py = Math.abs(this.PParam3);
        var ppx = px / py, ppy = py / px;
        if (this.PParam2 < 0 && this.PParam3 < 0)
            if (px > py)
                this.DirectionView = (ppx) > 2 ? 6 : 7;
            else
                this.DirectionView = (ppy) > 2 ? 0 : 7;
        if (this.PParam2 >= 0 && this.PParam3 < 0)
            if (px > py)
                this.DirectionView = (ppx) > 2 ? 2 : 1;
            else
                this.DirectionView = (ppy) > 2 ? 0 : 1;
        if (this.PParam2 >= 0 && this.PParam3 >= 0)
            if (px > py)
                this.DirectionView = (ppx) > 2 ? 2 : 3;
            else
                this.DirectionView = (ppy) > 2 ? 4 : 3;
        if (this.PParam2 < 0 && this.PParam3 >= 0)
            if (px > py)
                this.DirectionView = (ppx) > 2 ? 6 : 5;
            else
                this.DirectionView = (ppy) > 2 ? 4 : 5;
    };
